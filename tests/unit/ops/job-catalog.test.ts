...
describe("job catalog contract", () => {
  it("pins a workflow for every job and exposes it in definitions", () => {
    expect(JOB_CATALOG.every((row) => typeof row[3] === "string" && row[3].endsWith(".yml"))).toBe(true);
    expect(jobDefinition("collect_x_posts")).toMatchObject({ workflow: "collect-x.yml" });
  });

  it("rejects unknown fields and unsupported attribution", () => {
    expect(() => validateJobRequest({ name: "collect_x_posts", parameters: { nope: true } })).toThrow();
    expect(() => validateJobRequest({ name: "collect_x_posts", parameters: { orchestrator_workflow: "evil.yml" } })).toThrow();
  });
});
