...
describe("GitHub dispatcher",()=>{
 it("dispatches a catalog workflow with job name and command id",async()=>{const request=vi.fn(async()=>new Response(null,{status:204})); const commandId="11111111-1111-4111-8111-111111111111"; await dispatchGitHubWorkflow({owner:"o",repo:"r",token:"secret",request},{workflow:"trending.yml",jobName:"collect_github_trending",commandId}); expect(request).toHaveBeenCalledWith("https://api.github.com/repos/o/r/actions/workflows/trending.yml/dispatches",expect.objectContaining({method:"POST",body:JSON.stringify({ref:"main",inputs:{job_name:"collect_github_trending",command_id:commandId}})}));});
