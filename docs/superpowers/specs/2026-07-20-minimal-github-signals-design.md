...
- 同一个 Workflow 顺序执行 Daily、Weekly、Monthly 三个独立 Job 命令。
- 继续沿用当前 JobRunner 幂等键和 `github_trending_snapshots` 表。
- 每个 period 独立读取；一个 period 没有完整批次时，不影响其他 period。
- Skill 只读取该 period 最新的完整批次。完整批次要求同一 `period/language/captured_at` 内全部行 `collection_status='success'`。
- 新采集失败或只产生 partial 批次时，继续展示上一份完整批次。

### 展示

- 默认 Top 20；`--limit all` 返回完整批次，整数限制为 1–1000。
- `all` 表示一次返回日、周、月三个分区，每个分区分别应用 limit。
- 每个分区包含 `period`、`capturedAt`、`sourceUrl`、`stale` 和有序 `items`。
- `now - capturedAt > 36h` 时 `stale=true`，Markdown 明确标注数据可能过期。
- 每项包含排名、项目名、简介、作者、总 Star、周期新增 Star、GitHub URL、Homepage URL。
...
发现 Job 仍每 6 小时运行一次，搜索窗口只保留上述两条规则。新 Snapshot 写入 `candidate_rule_version='v2'`。报告候选计算也切换到相同的两条规则，避免系统内部出现双重口径。
...
Skill 只能调用部署管理的 CLI，必须原样保留排名、规则、采集时间、过期标记和来源链接。

## 错误处理

- 没有完整 Trending 批次时返回 `kind=not_found` 和 `reason=trending_unavailable`，不伪造空榜。
...
- Live：真实 GitHub Trending 三个 period 可采集入 Supabase；部署管理 CLI 与真实 Skill 调用能读取榜单和潜力项目，且输出时间与数据库事实一致。
