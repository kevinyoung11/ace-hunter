...
export function OpsJobDetail({ name }: { name: string }) {
  ...
  const [commandRows, setCommandRows] = useState<Record<string, unknown>[]>([]);
  useEffect(() => { void get<Job>(`/jobs/${encodeURIComponent(name)}`).then((value) => setJob({ state: "ready", data: value, fetchedAt: new Date() })).catch((e) => setJob({ state: "offline", data: null, error: e instanceof Error ? e.message : "job_failed", fetchedAt: new Date() })); void get<{commands: Record<string, unknown>[]}>(`/jobs/${encodeURIComponent(name)}/commands`).then((value) => setCommandRows(value.commands ?? [])).catch(() => setCommandRows([])); }, [name]);
  return <OpsFrame title={name} eyebrow="JOB DETAIL"><StateMessage state={job.state} error={job.error} fetchedAt={job.fetchedAt} />{job.data && <section className="ops-section"><dl><div><dt>执行器</dt><dd>{job.data.executor}</dd></div><div><dt>能力</dt><dd>{job.data.capability}</dd></div><div><dt>工作流</dt><dd>{job.data.workflow ?? "—"}</dd></div><div><dt>状态</dt><dd>{job.data.enabled && !job.data.pausedAt ? "启用" : "暂停/禁用"}</dd></div></dl><h2>最近命令</h2>{commandRows.length ? commandRows.map((command, index) => <p key={String(command.command_id ?? index)}>{String(command.command_id ?? "unknown")} · {String(command.action ?? "recorded")}</p>) : <p className="message">暂无命令记录。</p>}</section>}</OpsFrame>;
}
